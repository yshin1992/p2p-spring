...
public class PluploadUtil {
     ...
     * PDF文件路径
     */
    public static final String PDF_ROOT_PATH = CacheUtil.getProperty("agreement.root") + File.separator + "tmp"  + File.separator;
    ...
    public static String upload(Plupload plupload, File dir) throws IllegalStateException, IOException {
        //生成唯一的文件名
        String fileName = UUID.randomUUID().toString().toLowerCase().replace("-", "") + getUploadFileExt(plupload.getName());
        upload(plupload, dir, fileName);
        return fileName;
    }

    private static String getUploadFileExt(String fileName){
