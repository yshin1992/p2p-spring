...
import org.apis.dao.system.CategoryRepository;
			...
			if ((attrs != null) && (!attrs.isEmpty())) {
				...
				if (category.getConfiged().intValue() == 1) {
					for (CategoryAttr categoryAttr : attrs) {
						...
						logger.error("{}={}", categoryAttr.getAttrCd(),
								categoryAttr.getActualval());
					}
				} else
					try {
						...
						logger.error("{}={}", category.getCategoryCd(), attrList);
					} catch (Exception e) {
						logger.error("读取配置错误:{}", e.getMessage());
					}
			}
		}
		logger.error("1.环境初始化完成");
	}
